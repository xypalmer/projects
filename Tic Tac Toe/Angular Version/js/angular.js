...
tttApp.controller('tttController', function ($scope) {
  ...
  $scope.input = function(r, c) {
    if (tcounter % 2 == 1 ) {
        if ($scope.grid[r][c] == 0 || $scope.grid[r][c] == "p1b") {
            $scope.grid[r][c] = 1;
            $scope.insidebox = "X";
            tcounter++;
        }
        else if ($scope.grid[r][c] == "p2b" ) {
            $scope.grid[r][c] = 0;
            tcounter++;
        }
    }
    else if (tcounter % 2 == 0 ) {
        if ($scope.grid[r][c] == 0 || $scope.grid[r][c] == "p2b") {
            $scope.grid[r][c] = -1; 
            $scope.insidebox = "O";
            tcounter++;
        }
        else if ($scope.grid[r][c] == "p1b") {
            $scope.grid[r][c] = 0;
            tcounter++;
        }
    }
